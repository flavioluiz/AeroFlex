...
function example3_ver
    ...
    softPARAMS.isPINNED = 1; % engastar a asa para cálculo do equilíbrio? (estudos aeroelásticos)
    softPARAMS.vecFREEDEG = [0 0 0 0 1 0]; % type 0 to remove a body state degree of freedom [u v w p q r]
    softPARAMS.isGRAV = 0; % include gravity?
    softPARAMS.g = 0.0; % gravity in m/s^2    
...
function ap = carregaasavoadora(numele, amort, rigidez)
    ...
    motor1 = []; %numManete, posicao do motor[MEMB,ELM,ND], params
